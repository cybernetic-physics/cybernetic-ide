...
- `unitree_start_rpc_bridge` / `unitree_rpc_bridge_status` /
  ...
  `WaveHand`, and `ShakeHand`; `SetTaskId` also recognizes the official G1 arm
  action IDs from `G1ArmActionClient.action_map` and maps them onto the local
  named poses when there is a simulator pose for that action. Unknown task IDs
  remain explicit state-only intent. Continuous move mode (`SwitchMoveMode`),
  ...
  `sport.set_arm_task`, `sport.wave_hand`, `arm.execute_action`, or
