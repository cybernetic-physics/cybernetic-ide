...
Embedded Cybernetic IDE workspace item for viewing and lightly controlling the
Unitree G1 MuJoCo simulator through the Booster-style virtual robot harness.
...
HTTP GameControl endpoints on port `38383`. The Cybernetic Robot Viewer uses
HTTP status probes plus cached JPEG camera frames for the embedded viewport,
and sends camera-control updates separately so mouse input is not blocked on a
fresh MuJoCo render.
...
launching a debug Cybernetic IDE binary. The first workspace window will open
the robot viewer automatically inside the active editor pane.

Useful command palette actions:

- `cyber: open robot viewer`
